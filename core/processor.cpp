...
std::tuple<std::uint8_t, std::uint8_t, std::uint8_t, std::uint8_t, std::vector<std::uint8_t>> Processor::get_opcodes_at(
    std::uint16_t pc,
...
size_t Processor::emulate(std::uint8_t opcode, bool unbounded, size_t elapsed_cycles, size_t max_cycles)
{
    ...
    emulate_next_instruction:
        ...
        {
            BOOST_LOG_TRIVIAL(trace) << std::format("Stopping execution at PC={:04X}", m_effective_pc);
        ...
        {
            reg_a() = ~reg_a();
            reg_f() = (reg_f() & (SZPV_FLAG_MASK | C_FLAG_MASK)) | (reg_a() & YX_FLAG_MASK) | H_FLAG_MASK | N_FLAG_MASK;
        ...
        case CCF:
        {
            ...
            reg_f() = (reg_f() & (SZPV_FLAG_MASK | YX_FLAG_MASK)) | (c << H_FLAG_BIT) | (reg_a() & YX_FLAG_MASK) | (c ^ C_FLAG_MASK);
        ...
        case RLD_RRD:
        {
            ...
            y |= opcode == OPCODE_RLD ? (x << 4) | (reg_a() & 0x0f) : ((x & 0x0f) << 8) | ((reg_a() & 0x0f) << 4) | (x >> 4);
        ...
        case BIT_B_R:
        {
            ...
            reg_f() = (x ? 0 : Z_FLAG_MASK | PV_FLAG_MASK) | (x & S_FLAG_MASK) | (R(Z(opcode)) & YX_FLAG_MASK) | H_FLAG_MASK |
                      (reg_f() & C_FLAG_MASK);
        ...
        case BIT_B_INDIRECT_HL:
        {
            ...
            x &= 1 << Y(opcode);
            reg_f() = (x ? 0 : Z_FLAG_MASK | PV_FLAG_MASK) | (x & S_FLAG_MASK) | (d & YX_FLAG_MASK) | H_FLAG_MASK | (reg_f() & C_FLAG_MASK);
...
#ifdef TRACING
            BOOST_LOG_TRIVIAL(trace) << std::format("TRACE: Calling {:04X} from PC={:04X}", nn, pc - 3);
#endif
            pc = nn;
        ...
        case CALL_CC_NN:
        {
            if (test_cc(Y(opcode)))
            {
                ...
                m_memory.push(pc, elapsed_cycles);
#ifdef TRACING
                BOOST_LOG_TRIVIAL(trace) << std::format("TRACE: Calling {:04X} from PC={:04X} (cond)", nn, pc - 3);
#endif
                pc = nn;
        ...
        {
#ifdef TRACING
            BOOST_LOG_TRIVIAL(trace) << std::format("TRACE: Returning from PC={:04X}", pc - 1);
#endif
            pc = m_memory.pop(elapsed_cycles);
#ifdef TRACING
            BOOST_LOG_TRIVIAL(trace) << std::format("TRACE: Returning to PC={:04X}", pc);
#endif
        ...
        case RET_CC:
        {
            ...
            {
#ifdef TRACING
                BOOST_LOG_TRIVIAL(trace) << std::format("TRACE: Returning from PC={:04X} (cond)", pc - 1);
#endif
                pc = m_memory.pop(elapsed_cycles);
#ifdef TRACING
                BOOST_LOG_TRIVIAL(trace) << std::format("TRACE: Returning to PC={:04X}", pc);
