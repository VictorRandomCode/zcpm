...
System::System(std::unique_ptr<terminal::Terminal> p_terminal, const Config& behaviour) : m_hardware(std::move(p_terminal), behaviour)
{
}
...
bool System::load_binary(std::uint16_t base, std::string_view filename)
{
    ...
    BOOST_LOG_TRIVIAL(trace) << fmt::format("Reading {:d} bytes into memory at {:04X} from {}", filesize, base, filename);
