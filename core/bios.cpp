...
namespace ZCPM
{
  ...
  Bios::Bios(Hardware* p_hardware, Terminal::ITerminal* p_terminal) : m_phardware(p_hardware), m_pterminal(p_terminal)
  {
    ...
    BOOST_LOG_TRIVIAL(trace) << boost::format("BIOS jump table %04X..%04X, BIOS stubs %04X..%04X, DPH etc %04X..%04X") %
                                  m_discovered_base % (m_discovered_base + table_size * 3 - 1) % m_stubs_base %
                                  m_stubs_top % m_dph_base % m_dph_top;

    BOOST_LOG_TRIVIAL(trace) << boost::format("     dirbf=%04X hdblk=%04X chkhd1=%04X allhd1=%04X") % dirbf % hdblk %
                                  chkhd1 % allhd1;
  ...
  uint8_t Bios::fn_write(uint8_t /*deblocking*/)
  {
    ...
    BOOST_LOG_TRIVIAL(trace) << boost::format("Write TRACK:%04X,SECTOR:%04X from %04X") % m_track % m_sector % m_dma;
