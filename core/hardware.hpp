...
namespace ZCPM
{
  ...
  public:
    ...
    // Try to evaluate an expression such as 'foo1' where 'foo1' is a known label or perhaps 'foo2+23'.  Note that all
    // values are hexadecimal.  Returns a (success,value) pair, success=false means an evaluation failure.
    std::tuple<bool, uint16_t> evaluate_address_expression(const std::string& s) const;

    IDebuggable* get_idebuggable() const;

    // This is public, and is an ugly hack. The underlying problem is that the system we're emulated is tightly coupled
    // so it's hard to avoid the same patterns in emulation.
  ...
  private:
    ...
    // Addresses that we are watching; for now we just log their access, but longer-term we'll invoke a user-supplied
    // handler
