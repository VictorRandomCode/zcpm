...
public:
    ...
    [[nodiscard]] bool is_bios(std::uint16_t address) const;

    // Check if the specified address is within our custom BIOS implementation (and hence should be intercepted). If so, work out what the
    // intercepted BIOS call is trying to do and do whatever is needed, and then allows the caller to return to normal processing.  Returns
    ...
    [[nodiscard]] std::uint16_t fn_sectran(std::uint16_t logical_sector_number, std::uint16_t trans_table) const; // #16
...
private:
    ...
    // Where the current memory image has the start of the BIOS jump table which we determine by inspecting the WBOOT vector.
    std::uint16_t m_discovered_base{ 0 }; // Typically ends up being FC00

    // Calculated relative to the discovered BIOS base, this is the start of the address range where we point the set of BIOS vectors to
    // jump to, and we intercept this address range at runtime.
    ...
    // TODO: We currently use a single DPH which means that we treat everything as one big disk.  But later on we may need to have multiple
    // virtual disks, which means multiple DPH tables that all point to the same DPB. Needs more thought and/or experiments.
