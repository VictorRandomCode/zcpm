...
namespace ZCPM
{
  ...
  public:
    ...
    void show() const
    {
      ...
      BOOST_LOG_TRIVIAL(trace) << "  '" << m_raw_name << "' '" << m_name << "' Size=" << m_size
                               << " Sectors=" << m_sectors << " Extent=" << m_extent << " FirstBlock=" << m_first_block
                               << " [" << blocks.str() << " ] Exists:" << (m_exists ? 'Y' : 'N');
    }
  ...
  public:
      ...
      for (const auto& item : std::filesystem::directory_iterator(dir))
      {
        if (std::filesystem::is_regular_file(item.path()))
        {
          ...
          if (item.path().filename() != "zcpm.log") // TODO: Yuk
          {
            ...
            const auto first_block = m_next_block; // Need to remember first block across *all* entries/extents
    ...
    // location. Allows out of range values of 'n' which get mapped to E5 (inactive) file entries.  Note that this may
    // actually cause more than one entry to be created, in the case where a file requires multiple entries/extents.
    void format_directory_entry(uint8_t* base, uint16_t n) const
    {
      ...
      base[0x0C] =
        f.m_extent & 0x1F; // EX (TODO: mask & shift determined empirically; but where do magic values come from?)
            ...
            for (const auto& f : m_entries)
            {
              if (f.m_exists)
              {
                ...
                for (size_t i = 0; i < f.m_blocks.size(); i++)
                {
                  ...
                  {
                    BOOST_LOG_TRIVIAL(trace) << boost::format("Sector %02X:%02X is block %d offset %d within file %s") %
                                                  track % sector % block % static_cast<uint16_t>(offset) % f.m_raw_name;
                    try
                    {
    ...
    void flush_changed_file(const SectorInfo& value, uint16_t block, uint8_t offset, const Entry& f) const
    {
      ...
      // most file I/O is new files which uses different logic.
