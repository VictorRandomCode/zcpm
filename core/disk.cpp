...
public:
        ...
        : m_modified(true) // Needs to be flushed on shutdown because it is not (originally) a host filesystem file
    {
        ...
        m_raw_name = boost::to_lower_copy(boost::trim_right_copy(m_name.substr(0, 8)) + "." + boost::trim_right_copy(m_name.substr(8, 3)));
    ...
    std::uint16_t m_first_block = 0;     // What is the first block number of this file (across *all* of its extents/entries)
    std::vector<std::uint16_t> m_blocks; // Block indexes allocated for this entry for this file.  Always contiguous!!!
...
public:
    ...
    void format_directory_entry(std::uint8_t* base, std::uint16_t n) const
    {
        ...
        base[0x0C] = f.m_extent & 0x1F;        // EX (TODO: mask & shift determined empirically; but where do magic values come from?)
        base[0x0D] = 0x00;                     // S1 (Reserved, set to zero)
        base[0x0E] = (f.m_extent >> 5) & 0xFF; // S2 (TODO: mask & shift determined empirically; but where do magic values come from?)
