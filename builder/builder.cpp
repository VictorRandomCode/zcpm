...
{
    const auto value = std::string(std::getenv("HOME"));
...
std::unique_ptr<System> build_machine(int argc, char** argv)
{
    ...
    auto terminal = terminal::Type::PLAIN; // Terminal type
    ...
    try
    {
        ...
        desc.add_options()                                                                           //
            ...
            ("columns", po::value<int>(), "Terminal column count")                                   //
            ("rows", po::value<int>(), "Terminal row count")                                         //
            ...
            ("trace", "Detailed (very verbose) logging?")                                            //
        ...
        bdos_file_name = (vm.contains("bdosfile")) ? vm["bdosfile"].as<std::string>() : home_plus("zcpm/bdos.bin");
        config.bdos_sym = (vm.contains("bdossym")) ? vm["bdossym"].as<std::string>() : home_plus("zcpm/bdos.lab");
        if (vm.contains("bdosbase"))
        {
            bdos_file_base = vm["bdosbase"].as<std::uint16_t>();
        }
        if (vm.contains("wboot"))
        {
            wboot = vm["wboot"].as<std::uint16_t>();
        }
        if (vm.contains("fbase"))
        {
        ...
        keymap_file_name = (vm.contains("keymap")) ? vm["keymap"].as<std::string>() : home_plus("zcpm/wordstar.keys");
        if (vm.contains("columns"))
        {
        ...
        if (vm.contains("memcheck"))
        {
            config.memcheck = vm["memcheck"].as<bool>();
        }
        if (vm.contains("logbdos"))
        {
    ...
    // Put it all together
    auto p_machine(std::make_unique<System>(std::move(p_terminal), config));

    // The BDOS/CCP binary is built from Z80 source code which was reconstructed from a CP/M 2.2 disassembly plus a tweak or two. The
    // assembled binary is what is loaded here. ZCPM intercepts calls to the BIOS from the BDOS.
