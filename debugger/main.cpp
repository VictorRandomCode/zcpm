                                                                 ...
                                                                 const std::string& payload)
{
    ...
    else if (type == "passpoint")
    {
        ...
        {
            result = zcpm::DebugAction::create(zcpm::DebugAction::Type::PASSPOINT, a, location_string, count_string);
        }
    }
...
replxx::Replxx::completions_t hook_completion(const std::string& input, int& context_len, const std::vector<Command>& commands)
{
    ...
    for (const auto& command : commands)
    {
        for (const auto& command_verb : command.m_verbs)
        {
            if (verb == command_verb)
            {
                ...
                if ((words.size() >= command.m_min_word_count + 1) && (words.size() <= command.m_max_word_count + 1))
                {
                    try
                    {
...
auto run(Writer& writer, zcpm::System* p_machine, zcpm::IDebuggable* p_debuggable)
{
    const std::vector<Command> commands{
        ...
        Command{ { "quit" }, {}, 0, 0, "Exit from ZCPM", replxx::Replxx::Color::RED, [](const TokenVector& /*input*/) { return true; } },
        Command{ { "set" },
    ...
    rx.set_completion_callback([&commands](const std::string& input, int context_len) -> replxx::Replxx::completions_t
