...
namespace ZCPM::Console
{
  ...
  void Televideo::outch(char ch)
  {
    ...
    else // Anything else
    {
      if ((ch < ' ') || (ch > '~'))
      {
        BOOST_LOG_TRIVIAL(trace) << boost::format("Warning: unhandled CURSES %02X") % static_cast<unsigned short>(ch);
      }
  ...
  void Televideo::process_pending()
  {
    ...
    if (first == 'E')
    {
      // According to 4.9.2.3 in the Televideo reference, this "inserts a line consisting of fill characters at the
      // cursor position. This causes the cursor to move to the start of the new line and all following lines to move
      // down one line"
    ...
    // If we get here, that means that we appear to have an incomplete pending escape sequence, and the intent is that
